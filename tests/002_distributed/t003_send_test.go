...
// send compressed
// send exit (with custom reason. must be registered in sdf)

// TODO test send by process name with registered name in atom cache
// using sdf.RegisterAtom.
...
func (t *t3) TestSendRemoteExit(input any) {
	...
	// registered in sdf for sending over the network
...
func TestT3SendRemote(t *testing.T) {
	...
	node1, err := sparrow.StartNode("distT0node1SendRemote@localhost", options1)
	if err != nil {
		t.Fatal(err)
	}
	defer node1.Stop()
	...
	node2, err := sparrow.StartNode("distT0node2SendRemote@localhost", options2)
	if err != nil {
		t.Fatal(err)
	}
	defer node2.Stop()
