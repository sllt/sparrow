...
func TestT1RemoteNodeSpawn(t *testing.T) {
	...
	node1, err := sparrow.StartNode("distT1node1remotespawn@localhost", options1)
	if err != nil {
		t.Fatal(err)
	}
	defer node1.Stop()
	...
	node2, err := sparrow.StartNode("distT1node2remotespawn@localhost", options2)
	if err != nil {
		t.Fatal(err)
	}
	defer node2.Stop()
...
func TestT1ProcessRemoteSpawn(t *testing.T) {
	...
	options1.Log.Level = gen.LogLevelTrace
	options1.Security.ExposeEnvRemoteSpawn = true
	node1, err := sparrow.StartNode("distT1node1processremotespawn@localhost", options1)
	if err != nil {
		t.Fatal(err)
	}
	defer node1.Stop()
	...
	options2.Log.DefaultLogger.Disable = true
	options2.Security.ExposeEnvInfo = true
	node2, err := sparrow.StartNode("distT1node2processremotespawn@localhost", options2)
	if err != nil {
		t.Fatal(err)
	}
	defer node2.Stop()
