...
func (t *t9_sup_sofo_basic) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	spec.Type = actor.SupervisorTypeSimpleOneForOne
...
func (t *t9_sup_sofo_basic) TestBasicSOFO(input any) {
	if _, ok := input.(initcase); ok {
		...
		if err := t.StartChild("child_sofo1"); err != actor.ErrSupervisorChildUnknown {
			t.testcase.err <- errIncorrect
			return
		}
		...
		if err := t.DisableChild("child_sofo111"); err != actor.ErrSupervisorChildUnknown {
			t.testcase.err <- errIncorrect
			return
		}
	...
	if err := t.StartChild("child_sofo1"); err != actor.ErrSupervisorChildDisabled {
		t.testcase.err <- err
		return
	}
	t.testcase.err <- nil
...
func (t *t9_sup_sofo) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	spec.Type = actor.SupervisorTypeSimpleOneForOne
	spec.Children = []actor.SupervisorChildSpec{
		{
...
func TestT9SupervisorSOFO(t *testing.T) {
	...
	node, err := sparrow.StartNode("t9nodeSOFO@localhost", nopt)
	if err != nil {
		t.Fatal(err)
	}
