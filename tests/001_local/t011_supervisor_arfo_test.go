...
type t11_arfo struct {
	...
	supType  actor.SupervisorType
	testcase *testcase
}

func (t *t11_arfo) Init(args ...any) error {
	supType := args[0].(actor.SupervisorType)
	t.supType = supType
	return nil
}
...
func (t *t11_arfo) TestStrategy(input any) {
	...
	if t.supType == actor.SupervisorTypeAllForOne {
...
func (t *t11_arfo) TestSignificant(input any) {
	...
	strategy := t.testcase.input.(actor.SupervisorStrategy)
	...
	switch strategy {
	case actor.SupervisorStrategyTransient:
	...
	case actor.SupervisorStrategyTemporary:
	...
	case actor.SupervisorStrategyPermanent:
		reasons := []error{
...
func (t *t11_sup_arfo_basic) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	spec.Type = args[0].(actor.SupervisorType)
	spec.Children = []actor.SupervisorChildSpec{
		{
			Name:    "child_arfo",
...
func (t *t11_sup_arfo_basic) TestBasicARFO(input any) {
	if _, ok := input.(initcase); ok {
		...
		// start child
		if err := t.StartChild("child_arfo"); err != actor.ErrSupervisorChildRunning {
			t.testcase.err <- err
			return
		}
		...
		if err := t.StartChild("child_arfo111"); err != actor.ErrSupervisorChildUnknown {
			t.testcase.err <- errIncorrect
			return
		}
		...
		// add new child spec
		newChildSpec := actor.SupervisorChildSpec{
			Name:    "child_arfo3",
			Factory: factory_t11_child_arfo,
		}
		...
		if err := t.StartChild("child_arfo3"); err != actor.ErrSupervisorChildRunning {
			t.testcase.err <- err
			return
		}
		// check children. must be the same
		...
		if err := t.DisableChild("child_arfo111"); err != actor.ErrSupervisorChildUnknown {
			t.testcase.err <- errIncorrect
			return
		}
	...
	if err := t.StartChild("child_arfo"); err != actor.ErrSupervisorChildDisabled {
		t.testcase.err <- err
		return
	}
	t.testcase.err <- nil
...
func (t *t11_sup_arfo) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec

	t.supType = args[0].(actor.SupervisorType)
	spec.Type = t.supType
	t.strategy = args[1].(actor.SupervisorStrategy)
...
func (t *t11_sup_arfo) TestAFO(input any) {
	...
	case int:
		...
		t.waitStop = append(t.waitStop, stop)
		if t.strategy == actor.SupervisorStrategyTemporary {
			return
		}
...
func (t *t11_sup_arfo_significant) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	t.supType = args[0].(actor.SupervisorType)
	spec.Type = t.supType
	t.strategy = args[1].(actor.SupervisorStrategy)
...
func TestT11SupervisorAFO(t *testing.T) {
	...
	node, err := sparrow.StartNode("t11nodeAFO@localhost", nopt)
	if err != nil {
		t.Fatal(err)
	}

	popt := gen.ProcessOptions{}
	pid, err := node.Spawn(factory_t11_arfo, popt, actor.SupervisorTypeAllForOne)
	if err != nil {
		panic(err)
	}
	...
	t11casesARFO = []*testcase{
		...
		&testcase{"TestStrategy", actor.SupervisorStrategyTransient, nil, make(chan error)},
		&testcase{"TestStrategy", actor.SupervisorStrategyTemporary, nil, make(chan error)},
		&testcase{"TestStrategy", actor.SupervisorStrategyPermanent, nil, make(chan error)},
		&testcase{"TestSignificant", actor.SupervisorStrategyTransient, nil, make(chan error)},
		&testcase{"TestSignificant", actor.SupervisorStrategyTemporary, nil, make(chan error)},
		&testcase{"TestSignificant", actor.SupervisorStrategyPermanent, nil, make(chan error)},
...
func TestT11SupervisorRFO(t *testing.T) {
	...
	nopt.Log.DefaultLogger.Disable = true
	//nopt.Log.Level = gen.LogLevelTrace
	node, err := sparrow.StartNode("t11nodeRFO@localhost", nopt)
	if err != nil {
		t.Fatal(err)
	}

	popt := gen.ProcessOptions{}
	pid, err := node.Spawn(factory_t11_arfo, popt, actor.SupervisorTypeRestForOne)
	if err != nil {
		panic(err)
	}
	...
	t11casesARFO = []*testcase{
		...
		{"TestStrategy", actor.SupervisorStrategyTransient, nil, make(chan error)},
		{"TestStrategy", actor.SupervisorStrategyTemporary, nil, make(chan error)},
		{"TestStrategy", actor.SupervisorStrategyPermanent, nil, make(chan error)},
		{"TestSignificant", actor.SupervisorStrategyTransient, nil, make(chan error)},
		{"TestSignificant", actor.SupervisorStrategyTemporary, nil, make(chan error)},
		{"TestSignificant", actor.SupervisorStrategyPermanent, nil, make(chan error)},
