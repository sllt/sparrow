...
func (t *t10_sup_ofo_basic) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	spec.Type = actor.SupervisorTypeOneForOne
...
func (t *t10_sup_ofo_basic) TestBasicOFO(input any) {
	if _, ok := input.(initcase); ok {
		...
		// start child
		if err := t.StartChild("child_ofo"); err != actor.ErrSupervisorChildRunning {
			t.testcase.err <- err
			return
		}
		...
		if err := t.StartChild("child_ofo111"); err != actor.ErrSupervisorChildUnknown {
			t.testcase.err <- errIncorrect
			return
		}
		...
		if err := t.StartChild("child_ofo3"); err != actor.ErrSupervisorChildRunning {
			t.testcase.err <- err
			return
		}
		// check children. must be the same
		...
		if err := t.DisableChild("child_ofo111"); err != actor.ErrSupervisorChildUnknown {
			t.testcase.err <- errIncorrect
			return
		}
	...
	if err := t.StartChild("child_ofo"); err != actor.ErrSupervisorChildDisabled {
		t.testcase.err <- err
		return
	}
	t.testcase.err <- nil
...
func (t *t10_sup_ofo) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	spec.Type = actor.SupervisorTypeOneForOne
	spec.Children = []actor.SupervisorChildSpec{
		{
	...
	spec.Restart.Strategy = args[0].(actor.SupervisorStrategy)
...
func (t *t10_sup_ofo_significant) Init(args ...any) (actor.SupervisorSpec, error) {
	var spec actor.SupervisorSpec
	spec.Type = actor.SupervisorTypeOneForOne
	spec.Children = []actor.SupervisorChildSpec{
		{
...
func TestT10SupervisorOFO(t *testing.T) {
	...
	node, err := sparrow.StartNode("t10nodeOFO@localhost", nopt)
	if err != nil {
		t.Fatal(err)
	}
