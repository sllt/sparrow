...
func (m *metrics) Init(args ...any) error {
	if err := sdf.RegisterTypeOf(MessageMetrics{}); err != nil {
		if err != gen.ErrTaken {
			return err
		}
	}
...
func (m *metrics) send() {
	var msrv = "metrics.sparrow.services"
	...
	msg := MessageMetrics{
		...
		SparrowVersion: m.Node().FrameworkVersion().String(),
	...
	if err := sdf.Encode(msg, payload, sdf.Options{}); err != nil {
